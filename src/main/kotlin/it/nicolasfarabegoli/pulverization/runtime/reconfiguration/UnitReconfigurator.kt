    ...
    private suspend fun spawnRulesObserver() {
        ...
        suspend fun <S : Any> ReconfigurationEvent<S>.checkRule(newConfig: NewConfiguration) {
            ...
            events.collect { event ->
                if (targetComponent in localComponents && predicate(event)) {
                    val result = runCatching {
                        ...
                        onReconfigurationEvent(ReconfigurationSuccess(event))
                    }
                    result.getOrElse { onReconfigurationEvent(FailOnReconfiguration(event, it)) }
                } else { onReconfigurationEvent(SkipCheck(event)) }
            }
